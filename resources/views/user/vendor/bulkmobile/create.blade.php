                ...
                <div class="card custom-card overflow-hidden">
                    <div class="card-body">
                        <div class="page-header mt-2 mb-4">
                            <div>
                                <h4 class="main-content-title">New Mobile Bulk Upload</h4>
                            </div>
                        </div>
                        <form method="POST" action="{{Route('vendor.bulk.mobile.save')}}" accept-charset="UTF-8">
                            @csrf
...
<script>
   $(document).ready(function() {
    ...
    $(document).on('change', '#brand', function() {
        var brand = $(this).val();
        var url = "{{ route('vendor.bulk.mobile.model.search') }}";
        console.log('url',url);
        $.ajax({
            url: url,
            method: 'GET',
