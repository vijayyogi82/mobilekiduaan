    ...
    <div class="item-box">
        <div class="item-image">
            @php
                ...
                $id = $mobile->id;
            @endphp
            @if($mobile->accessory->image)
                <a href="{{ route('vendor.assessries', ['name' => $mobileName, 'id' => $id]) }}">
                    <img src="{{ asset(explode(';',$mobile->accessory->image)[0])}}" loading="lazy" alt="{{$mobile->accessory->model_name}}" data-ll-status="loaded">
                </a>
            @else
            <a href="{{ route('vendor.assessries', ['name' => $mobileName, 'id' => $id]) }}">
        ...
        <div class="item-name">{{$mobile->accessory ? $mobile->accessory->model_name : ''}}</div>
    </div>
</div>
@endforeach